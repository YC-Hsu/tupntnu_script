...
if [ $# != 6 ]; then
   echo "Usage: $0 [options] <phones-txt> <MS-info> <ali-dir> <nnforward-exp> <feats-scp> <corpus-dir>";
   echo "e.g.: $0 data/lang/phones.txt \\"
   echo "\texp/tri3a_MPDN4H1024_test_ali \\"
   echo "\t/mnt/corpus/TUP_NTNU/MS.info \\"
...
nnet-forward --no-softmax=true --prior-scale=1.0 \
			...
			scp:$feats_scp ark,t:$corpus_dir/tmp/feats_nnf.txt 

python local/py_code/tupntnu_make_phone_level_corpus.py $phones_txt $MS_info $corpus_dir/tmp/state_and_phone.txt \
													$corpus_dir/tmp/ali.txt $corpus_dir/tmp/feats_nnf.txt \
													$corpus_dir/phone_level_corpus.txt
