...
if len(sys.argv) != 4 :
	...
	while True:
		...
		for i in range(1, len(list)-1) :
			if list[i] != '':
				for j in range(0, len(dict[list[i]])) :
					reg.append(dict[list[i]][j])
		ans[str(list[0])] = reg
	...
	tonal_syllable_error_rate = (float(error_count)/float(total_count))*100
	print "tonal_syllable_error_rate:" + str(round(tonal_syllable_error_rate,2)) + "%"
